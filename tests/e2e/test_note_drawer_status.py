...
def test_note_autosave_shows_saved_status(ace_server, browser_name):
    with sync_playwright() as p:
        ...
        try:
            ...
            page.locator("#note-textarea").fill("Autosave feedback")

            expect(page.locator("#note-status")).to_have_text("Saved")
        finally:
            browser.close()


@pytest.mark.parametrize("browser_name", browser_params())
def test_note_autosave_does_not_replace_long_note_warning(ace_server, browser_name):
    with sync_playwright() as p:
        browser = getattr(p, browser_name).launch()
        try:
            page = browser.new_page()
