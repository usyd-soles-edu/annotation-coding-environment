    ...
    "src/ace/static/js/codebook_headless_tree_source.js"() {
      ...
      (function() {
        ...
        function codebookMutationValues(values) {
          const next = __spreadValues({}, values || {});
        ...
        function rowForItem(item) {
          ...
          itemProps.onKeyDown = function(event) {
            if (event.key === "ArrowRight" && !event.altKey && !event.shiftKey && !event.ctrlKey && !event.metaKey && typeof window.aceCodingKeyboard?.returnToSource === "function") {
              event.preventDefault();
              event.stopPropagation();
