  ...
  (function() {
    ...
    function rowForItem(item) {
      ...
      itemProps.onKeyDown = function(event) {
        if (event.key === "ArrowRight" && isCodingMode() && !event.altKey && !event.shiftKey && !event.ctrlKey && !event.metaKey && typeof window.aceCodingKeyboard?.returnToSource === "function") {
          event.preventDefault();
          event.stopPropagation();
        ...
        if (modePolicy().autoViewOnFocus && data.kind === "code" && plainTreeKey(event) && (event.key === "ArrowDown" || event.key === "ArrowUp" || event.key === "Home" || event.key === "End")) {
          event.preventDefault();
          event.stopPropagation();
        ...
        if (data.kind === "code" && (event.key === " " || event.code === "Space") && plainTreeKey(event) && modePolicy().spaceOnCode === "view") {
          event.preventDefault();
          event.stopPropagation();
          viewCode(item, { pushHistory: true });
          return;
    ...
    function createController() {
      ...
      function focusTreeItem(element, options) {
        ...
        if (!item) return;
        options = options || {};
        const shouldActivate = Object.prototype.hasOwnProperty.call(options, "activate") ? options.activate : isAuditMode() && item.getItemData?.().kind === "code";
        focusTreeItemInstance(item, {
